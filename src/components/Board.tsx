...
export default function Board(props: BoardProps) {
  ...
  const todoTiles: Array<JSX.Element> = props.layout.columns[0].cards.map(
    (item, index) => {
      const itemId = uuid();
      return (
  ...
  const progressTiles: Array<JSX.Element> = props.layout.columns[1].cards.map(
    (item, index) => {
      const itemId = uuid();
      return (
        <Draggable key={itemId} draggableId={itemId} index={index}>
  ...
  const doneTiles: Array<JSX.Element> = props.layout.columns[2].cards.map(
    (item, index) => {
      const itemId = uuid();
      return (
        <Draggable key={itemId} draggableId={itemId} index={index}>
        ...
        {(provided: DroppableProvided, snapshot: DroppableStateSnapshot) => (
          ...
          >
            <div className="board--column">
              ...
              <hr />
              {todoTiles.length > 0 && (
                <div className="board--tileWrapper">{todoTiles}</div>
              )}
              <div className="board--addButton">
                <button
                  onClick={() => props.showPrompt("New tile:", "addTile")}
                >
                  Add new tile
                </button>
              </div>
          ...
          >
            <div className="board--column">
              ...
              {progressTiles.length > 0 && (
                <div className="board--tileWrapper">{progressTiles}</div>
              )}
            </div>
            {provided.placeholder}
          </div>
        ...
        {(provided: DroppableProvided, snapshot: DroppableStateSnapshot) => (
          ...
          >
            <div className="board--column">
              ...
              <hr />
              {doneTiles.length > 0 && (
                <div className="board--tileWrapper">{doneTiles}</div>
              )}
            </div>
            {provided.placeholder}
          </div>
